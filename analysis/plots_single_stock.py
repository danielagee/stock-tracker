class PortfolioPlot:
    ...
    for ticker in merged.df_merged.tickers:
        # Basic candlestick plot using date, open, high, low, and close.
        fig.add_trace(go.Candlestick(x=merged.df_merged['Date'], open=merged.df_merged[f'Open-{ticker}'],
                                     high=merged.df_merged['High'], low=merged.df_merged['Low'],
                                     close=merged.df_merged['Close']), secondary_y=False)
